...
const Navigation = () => {
  ...
  // Clear pulse when visiting My Notes
  useEffect(() => {
    if (location.pathname === "/my-notes" && hasNewNote) {
      clearNewNote();
    }
          ...
          <div className="flex-1 overflow-x-auto nav-scrollbar">
            <div className="flex items-center gap-1 min-w-max py-1">
              {tabs.slice(1).map((tab) => {
                const isNotesTab = tab.path === "/my-notes";
                const shouldPulse = isNotesTab && hasNewNote;

                return (
                  <button
                    key={tab.path}
                    onClick={() => navigate(tab.path)}
                    className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-body tracking-wide whitespace-nowrap transition-all duration-200 ${
